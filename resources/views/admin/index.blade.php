        ...
        <div class="rounded-lg w-full p-3 bg-white ring-offset-0 ring-0 shadow-lg">
            @if ($blogs->count() > 0)
                @for ($i = 0; $i<count($blogs); $i++)
                    <a href="{{'blog/'}}{{$blogs[$i]->id}}/{{$blogs[$i]->title}}" class="w-full my-2 hover:opacity-75">
                        ...
                        <div>
                            ...
                            @endif
                            <a href="{{'blog/'}}{{$blogs[$i]->id}}/{{$blogs[$i]->title}}" class="hover:underline"><i class="fa fa-book-reader m-1"></i>Read Now</a>
                            @if($blogs[$i]->author == Auth::id())
                                <a href="{{'/editpost/'}}{{$blogs[$i]->id}}/{{$blogs[$i]->title}}" class="hover:underline mx-1 w-full"><i class="fa fa-pen m-1"></i>Edit</a><a href="{{'/deletepost/'}}{{$blogs[$i]->id}}/{{$blogs[$i]->title}}" class="hover:underline mx-1"><i class="fa fa-trash m-1"></i>Delete</a>
                            @endif
                            </p>
                        </div>
            ...
            @else
                ...
                <p class="w-full opacity-70 text-center"><a href="{{route('createblog')}}" class="text-sm hover:underline p-3 font-semibold">create post</a></p>
            @endif
        </div>
    </div>
    @if ($recent->count())
        <div class="w-full md:w-2/6 p-2 md:h-screen md:overflow-scroll md:float-right md:ml-auto">
            ...
            @for ($k=0; $k < count($recent); $k++)
                <div class="bg-white rounded-lg mb-3 ring-offset-0 ring-0 shadow-lg ">   
                    <a href="{{'blog/'}}{{$recent[$k]->id}}/{{$recent[$k]->title}}" class="block w-full hover:opacity-70">
                        ...
                        <div>
                            ...
                            @if (date('d', strtotime($recent[$k]->created_at)) === date('d', time()))
                                ...
                                @endif
                            <a href="{{'blog/'}}{{$recent[$k]->id}}/{{$recent[$k]->title}}" class="inline hover:underline"><i class="fa fa-book-reader m-1"></i>Read Now</a></p>
                        </div>
