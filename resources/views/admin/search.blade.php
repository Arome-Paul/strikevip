...
@section('content')
    <h2 class="font-bold text-lg p-3 capitalize">Search Result For '{{$keyword}}' ({{$users->count()}})</h2>

    <div>
        <form action="{{route('search')}}" method="post" class="bg-white w-full p-3 py-5 ring-0 ring-offset-0 shadow-lg rounded-lg">
            @csrf
            <input type="search" name="search" value="{{$keyword}}" placeholder="Search For User">
            <button class="bg-blue-800 p-2 px-4 text-white font-semibold rounded-lg transition-all hover:bg-opacity-80">Search</button>
        </form>
        <a href="{{route('admin.users')}}" class="px-3 font-semibold text-blue-600 hover:text-blue-800 hover:underline">View All Users</a>

        @if($users->count() > 0)
            <form action="" method="post" class="my-5">
                    @csrf
                    @method('put')
                <table class="w-full table bg-white ring-0 ring-offset-0 shadow-lg rounded-lg p-3 border-solid border-b-2 border-b-gray-300">
                    <thead>
        ...
        @else
                <p class="capitalize flex items-center justify-center w-full text-2xl pt-16"><i class="fa fa-search text-3xl"></i>Search Result not Found</p>
        @endif
